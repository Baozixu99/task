...
use os::arch::get_bad_addr;
use os::mm::{
    copy_from_user, copy_to_user, translated_ref, translated_refmut, try_get_from_user,
};
use os::syscall::errno::*;
use super::manager::wait_with_timeout;
use super::{block_current_and_run_next, exit_current_and_run_next, exit_group_and_run_next};
use os::timer::TimeSpec;
use os::{config::*, signal_type};
                ...
                if act.flags.contains(SigActionFlags::SA_SIGINFO) {
                    copy_to_user(
                        ...
                        &UserContext {
                            flags: 0,
                            link: 0,
                            stack: os::task::signal::SignalStack::new(sig_sp, sig_size),
